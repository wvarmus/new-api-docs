...
* **API 基础地址 (Base URL)**：`https://api.wxxingheai.com/v1`
* **协议支持**：仅支持 HTTPS 加密传输。

> **[图片备注：API 地址配置示例]**
> 展示常用的 AI 客户端（如 Cherry Studio 或 NextChat）中，“自定义接口地址”栏填入上述地址的特写截图，提醒用户不要漏掉末尾的 `/v1`。
...
> **[图片备注：网络响应延迟对比图]**
> 展示一张对比图，左边是普通请求，右边是 Stream 流式请求，体现出流式请求在“首字响应时间”上的极大优势。
...
```bash
curl [https://api.wxxingheai.com/v1/chat/completions](https://api.wxxingheai.com/v1/chat/completions) \
