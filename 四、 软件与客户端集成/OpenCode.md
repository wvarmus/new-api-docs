...
```bash
export OPENAI_API_BASE="https://api.wxxingheai.com/v1"
...
4. **Base URL**：填入 `https://api.wxxingheai.com/v1`
5. **API Key**：填入您的无限星河 `sk-` 密钥。

> **[图片备注：OpenCode 终端 TUI 界面截图]**
> 展示 OpenCode 极具特色的暗色调终端排版，高亮显示底部的 `build` 和 `plan` 模式切换标签，以及对话框。
