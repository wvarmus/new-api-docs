...
NextChat 的核心功能之一是其内置的**“面具 (Masks)”**系统，也就是预设提示词（Prompts）。

* **使用方法**：在新建对话时点击右上角的“面具”图标，您可以选择数十种预设的系统级专家身份（如：小红书写手、英翻中专家、Linux 终端模拟器等）。
* **搭配建议**：不同的面具对模型的逻辑要求不同。例如使用“代码专家”面具时，建议在模型广场选择适合代码分析或推理任务的模型。
...
* **日常面具/快速问答**：可选择轻量或高速模型。
* **专业沉浸写作**：可选择适合创意写作和长上下文任务的模型。
* **复杂数学/代码推理**：可选择具备推理能力的模型。
