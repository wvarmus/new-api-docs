...
Claude Code 是 Anthropic 官方发布的命令行编程助手（CLI）。它不同于普通的对话插件，能够直接在您的本地终端内深入项目目录，执行代码分析、自动化 Bug 修复、运行测试并直接提交 Git 变更。通过无限星河AI 的网关，您可以按平台当前支持能力使用对应模型的“Thinking（深度思考）”模式。
...
1. **思维链（Thinking）能力支持**：平台会根据当前模型与接口能力支持对应的深度思考模式。
2. **调用更顺畅**：通过分布式中转与线路调度，提升 CLI 工具在频繁读取文件时的响应体验。
3. **高频调用更稳定**：Claude Code 在索引项目时会产生密集请求，企业级节点可提升高频调用场景下的稳定性，具体限制以线路配置、账户状态和上游服务状态为准。
...
* **工程重构与复杂 Debug**：可在模型广场选择适合代码分析和长上下文任务的模型。
* **日常咨询与简单修改**：可选择轻量或高速模型，兼顾响应速度与成本。
