...
通过连接无限星河AI 的高可用网关，您可以为 Crush 接入适合代码分析、工具调用和长上下文任务的模型能力。
...
* **解决方案**：请务必确保您的无限星河账户余额充足，并在进行大规模重构时选择适合代码分析和长上下文任务的模型。
...
* **本地探索与文件整理**：可选择轻量或高速模型，兼顾响应速度与成本。
* **复杂架构与多文件重构**：可选择适合代码分析、长上下文和工具调用任务的模型。
* **算法与底层排错**：可选择具备推理能力的模型。
