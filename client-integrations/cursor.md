...
Cursor 是一款基于 VS Code 开发的深度集成 AI 的代码编辑器。它不仅支持基础的对话，还能理解您的整个项目代码库（Codebase），提供代码补全和重构建议。通过无限星河AI，您可以按任务场景选择适合的推理、代码或高速模型。
...
这是 Cursor 的核心功能之一。您可以描述一个功能（例如：“帮我写一个登录页面的 API 接口”），AI 会直接在编辑器中生成代码，您可以点击 `Apply` 直接应用。
...
* **代码重构与 Debug**：可在模型广场选择适合代码分析和长上下文任务的模型。
* **复杂架构设计**：可选择具备推理能力的模型。
* **日常脚本/注释生成**：可选择轻量或高速模型，兼顾响应速度与成本。
