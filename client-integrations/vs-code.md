...
对于使用 VS Code 的开发者，**Continue** 是生态成熟、配置灵活的 AI 编程助手插件。通过接入无限星河AI，您可以在 VS Code 中使用代码补全、对话提问、代码重构以及读取本地项目代码库等能力。
...
> *注意：由于无限星河AI 兼容 OpenAI 常用接口格式，这里的 `provider` 必须填写为 `openai`，无论是调用 Claude 还是 Gemini。*
...
* **复杂架构与深层逻辑重构**：可在模型广场选择适合代码分析和长上下文任务的模型。
* **算法与断点 Debug**：可选择具备推理能力的模型。
* **实时打字自动补全**：可选择轻量或高速模型，兼顾响应速度与成本。
