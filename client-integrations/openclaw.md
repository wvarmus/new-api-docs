...
OpenClaw 是一款开源 AI 智能体框架 (Agent Framework)。它打破了传统 AI“一问一答”的限制，可以作为您的数字助手在后台运行，不仅能接入 Telegram、Slack、Discord 等常用通讯软件，还能通过技能库（Skills）帮您处理邮件、管理日历或运行代码。

通过无限星河AI 的高可用网关，您可以为 OpenClaw 接入适合工具调用、长上下文和复杂任务的模型能力。
...
* **解决方案**：建议在模型广场选择适合长上下文和工具调用任务的模型，避免使用过轻量的模型承担复杂 Agent 任务。
...
* **日常消息处理 & 新闻摘要**：可选择响应更快、成本更低的轻量模型。
* **复杂工具调用 & 本地文件操作**：可选择适合长上下文和工具调用任务的模型。
* **高风险代码自动化修复**：可选择具备推理能力的模型，并在执行前人工确认关键变更。
